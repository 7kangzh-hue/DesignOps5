...
【日期归属规则（高优先级）】
1. Manager Notes 中每一条记录，必须严格保留其原始日期归属。
2. 某条记录如果属于周五，就只能出现在周五的“本周工作流水”中，严禁挪动到周四或其他日期。
3. 允许对同一天内的上午/下午进行合理编排和润色，但不允许跨天调整。
4. “随机穿插混合”仅限于同一天内部的条目顺序混合，不代表可以跨日期打乱。
5. 若某条原始记录未注明上午/下午，可根据工作性质合理分配到该日期的上午或下午，但日期本身不得改变。
